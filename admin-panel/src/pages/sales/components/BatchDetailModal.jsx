...
const TabButton = ({ active, children, onClick, ariaLabel }) => (
  <button
    type="button"
    className={`inline-flex min-h-11 items-center justify-center rounded-full border px-4 py-2 text-sm font-semibold transition-all duration-200 ${
      active
        ? 'border-gold-500/40 bg-gold-500/10 text-gold-100'
        : 'surface-panel-soft panel-border text-primary hover:bg-gold-500/8'
    }`}
    onClick={onClick}
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain [scrollbar-gutter:stable] px-5 py-5 md:px-6">
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              {!hasItems ? <SnapshotNotice>This batch does not have any items yet.</SnapshotNotice> : null}

              <div className="rounded-2xl surface-panel-faint panel-border p-2">
                <div className="flex flex-wrap gap-2">
                  <TabButton active={activeTab === 'overview'} onClick={() => setActiveTab('overview')} ariaLabel="Show overview">Overview</TabButton>
                  <TabButton active={activeTab === 'items'} onClick={() => setActiveTab('items')} ariaLabel="Show items">Items</TabButton>
                  <TabButton active={activeTab === 'history'} onClick={() => setActiveTab('history')} ariaLabel="Show history">History</TabButton>
                </div>
              </div>
              ...
              {activeTab === 'overview' ? (
                <div className="space-y-4">
                  <section className="rounded-3xl surface-panel-soft panel-border p-5 shadow-[0_10px_30px_rgba(0,0,0,0.04)]">
                    <SectionTitle title="Overview" description="Primary batch information." />
                    <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                      ...
                      <DetailField label="Created" value={formatDateTimeOrDash(batch?.createdAt)} />
                    </div>
                  </section>

                  <section className="rounded-3xl surface-panel-soft panel-border p-5 shadow-[0_10px_30px_rgba(0,0,0,0.04)]">
                    <SectionTitle title="Totals summary" description="Operational totals captured for this batch." />
                  ...
                  <section className="rounded-3xl surface-panel-soft panel-border p-5 shadow-[0_10px_30px_rgba(0,0,0,0.04)]">
                    <SectionTitle title="Batch actions" description="Submit, finalize, or reopen this batch as the workflow progresses." />
                      ...
                      {canSubmit ? (
                        <div className="space-y-2">
                          <button
                            ...
                            disabled={actionBusy || !hasItems}
                            aria-disabled={actionBusy || !hasItems}
                          >
                            Submit batch
                          </button>
                          {!hasItems ? <div className="text-xs text-muted">Add at least one item before submitting.</div> : null}
                        </div>
                      ) : null}
                      {canFinalize ? (
                        <button type="button" className={buttonStyles.primary} onClick={() => setActiveAction('finalize')} disabled={actionBusy}>
                          Finalize batch
                        </button>
                      ) : null}
                      {canReopen ? (
                        <button type="button" className={buttonStyles.secondary} onClick={() => setActiveAction('reopen')} disabled={actionBusy}>
                          Reopen batch
                        </button>
                      ) : null}
                    </div>
                      ...
                      <div className="mt-4 rounded-2xl surface-panel-faint panel-border p-4">
                        ...
                        {activeSummaryAction === 'submit' ? (
                          <div className="space-y-3">
                            <div className="text-sm text-muted">Submitting moves this batch to the next workflow step.</div>
                            <div className="flex flex-wrap gap-3">
                              <button type="button" className={buttonStyles.primary} onClick={() => runAction('submit')} disabled={actionBusy}>
                                {actionBusy ? <><LoadingSpinner />Submitting...</> : 'Confirm submit'}
                              </button>
                              <button type="button" className={buttonStyles.secondary} onClick={() => setActiveAction(null)} disabled={actionBusy}>
                                Cancel
                              </button>
                            </div>
                          </div>
                        ...
                        {activeSummaryAction === 'finalize' ? (
                          <div className="space-y-3">
                            <div className="text-sm text-muted">Finalizing locks the current revision until an admin reopens it.</div>
                            <div className="flex flex-wrap gap-3">
                              <button type="button" className={buttonStyles.primary} onClick={() => runAction('finalize')} disabled={actionBusy}>
                                {actionBusy ? <><LoadingSpinner />Finalizing...</> : 'Confirm finalize'}
                              </button>
                              <button type="button" className={buttonStyles.secondary} onClick={() => setActiveAction(null)} disabled={actionBusy}>
                                Cancel
                              </button>
                        ...
                        {activeSummaryAction === 'reopen' ? (
                          <div className="space-y-3">
                            <div className="text-sm text-muted">Reopening creates a new revision. Previous finalized history remains preserved.</div>
                            <div className="field">
                              <label className="field-label" htmlFor="batch-reopen-reason">Reopen reason</label>
                              <textarea
                            ...
                            <div className="flex flex-wrap gap-3">
                              <button type="button" className={buttonStyles.primary} onClick={() => runAction('reopen')} disabled={actionBusy}>
                                {actionBusy ? <><LoadingSpinner />Reopening...</> : 'Confirm reopen'}
                              </button>
                              <button type="button" className={buttonStyles.secondary} onClick={() => setActiveAction(null)} disabled={actionBusy}>
                                Cancel
                              </button>
              ...
              {activeTab === 'items' ? (
                <section className="rounded-3xl surface-panel-soft panel-border p-5 shadow-[0_10px_30px_rgba(0,0,0,0.04)]">
                  <SectionTitle title="Child items" description="Item-level audit records inside this batch." />
                  {hasItems ? (
                    <div className="mt-4 overflow-x-auto overscroll-x-contain [scrollbar-gutter:stable]">
                      <table className="w-full min-w-[1300px] text-left">
                        ...
                        <tbody className="divide-y divide-[var(--jsm-border)]">
                          {items.map((sale) => {
                            const calculation = sale?.calculationSnapshot && typeof sale.calculationSnapshot === 'object' ? sale.calculationSnapshot : {}
                            const settlementInputs = sale?.settlementInputs && typeof sale.settlementInputs === 'object' ? sale.settlementInputs : {}
                            const parsedSnapshot = sale?.parsedSnapshot && typeof sale.parsedSnapshot === 'object' ? sale.parsedSnapshot : {}
                            const parsedDisplay = parsedSnapshot?.display && typeof parsedSnapshot.display === 'object' ? parsedSnapshot.display : parsedSnapshot
                            const parsedItem = parsedDisplay?.item && typeof parsedDisplay.item === 'object' ? parsedDisplay.item : {}
                            ...
                            return (
                              <tr key={sale._id} className="hover:bg-[var(--jsm-panel-bg-faint)]">
                                <td className="px-4 py-3 font-mono text-xs text-muted whitespace-nowrap">{valueOrDash(sale?.ref)}</td>
                                ...
                                <td className="px-4 py-3 whitespace-nowrap text-primary"><ChildRowBadge tone={getEntryTone(sale?.entryMode)}>{entryMode}</ChildRowBadge></td>
                                <td className="px-4 py-3 text-right whitespace-nowrap text-primary">{valueOrDash(sale?.revisionAdded)}</td>
                                <td className="px-4 py-3">
                                  <div className="flex flex-wrap gap-2">
                                    ...
                                    {!duplicate && !requiresReview && !wasOverridden ? <ChildRowBadge tone="neutral">OK</ChildRowBadge> : null}
                                  </div>
                                </td>
                    ...
                    <div className="mt-4"><SnapshotNotice>This batch does not have any items yet.</SnapshotNotice></div>
                  )}
                </section>
              ) : null}
              ...
              {activeTab === 'history' ? (
                <div className="space-y-4">
                  <section className="rounded-3xl surface-panel-soft panel-border p-5 shadow-[0_10px_30px_rgba(0,0,0,0.04)]">
                    <SectionTitle title="Current revision" description="The latest finalized or active revision summary." />
                    <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                      ...
                      <DetailField label="Status" value={valueOrDash(formatBatchStatusLabel(status))} />
                      <DetailField label="Finalized At" value={formatDateTimeOrDash(currentRevision?.finalizedAt || batch?.finalizedAt)} />
                  ...
                  <section className="rounded-3xl surface-panel-soft panel-border p-5 shadow-[0_10px_30px_rgba(0,0,0,0.04)]">
                    <SectionTitle title="Revision history" description="Collapsed rows preserve the full history without crowding the modal." />
                    <div className="mt-4 space-y-3">
                      {hasRevisions ? (
                        revisionHistory.map((revision) => (
                          <details key={revision.revision} className="group rounded-2xl surface-panel-faint panel-border p-4">
                            <summary className="flex cursor-pointer list-none items-center justify-between gap-4">
                              ...
                              <div className="text-right text-[11px] uppercase tracking-[0.18em] text-muted group-open:text-heading">
                                {formatDateTimeOrDash(revision.finalizedAt)}
                              </div>
                            </summary>
                            <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                        ...
                        <div className="rounded-2xl border border-dashed panel-border surface-panel-soft px-4 py-4 text-sm text-muted">No historical revisions yet.</div>
                      )}
                    </div>
                  </section>
                </div>
